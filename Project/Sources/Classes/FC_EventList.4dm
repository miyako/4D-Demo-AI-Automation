...
Function _loadEvents($filter : Text)
	...
	Case of 
		...
		: ($filter="weather")
			...
			End if 
		: ($filter="email")
			// Events with at least one unread linked email
			var $emailedIDs : Collection:=ds.Email.query("emailStatus = :1 AND linkedEventID != :2"; "unread"; "").toCollection("linkedEventID")
			If (This.showPast)
				$selection:=ds.Event.query("ID IN :1"; $emailedIDs).orderBy("eventDate ASC")
			Else 
				$selection:=ds.Event.query("ID IN :1 AND eventDate >= :2"; $emailedIDs; $today).orderBy("eventDate ASC")
			End if 
		Else 
			If (This.showPast)
	...
	// Pre-compute which event IDs have unread emails (for icon column)
	var $unreadEmailIDs : Collection:=ds.Email.query("emailStatus = :1 AND linkedEventID != :2"; "unread"; "").toCollection("linkedEventID")
	...
	For each ($evt; $selection)
		...
		// Email icon: 📧 if this event has an unread linked email
		$item.emailIcon:=Choose($unreadEmailIDs.indexOf($evt.ID)>=0; "📧"; "")
		$result.push($item)
	End for each 
...
Function _updateFilterCounts()
	...
	OBJECT SET TITLE(*; "btn_filter_email"; "📧 Email ("+String($emailCount)+")")
